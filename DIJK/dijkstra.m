...
%  Below = neighbour < 1;
%  above = neighbour > [h, w, d];
%  boundary = Below + above;
% [deleted, ~] = find(boundary > 0);
% neighbour(deleted,:) = [];
% neighbour_ind = sub2ind(size(map.occgrid),neighbour(:,1),neighbour(:,2), neighbour(:,3));
% [deleted, ~] = find(map.occgrid(neighbour_ind) > 0);
% neighbour(deleted,:) = [];
...
%             v=map.res_xyz.*abs(neighbour(ii,:)-goal_sub);
            H(neighbour_ind)=g(neighbour_ind)+norm(map.res_xyz.*abs(neighbour(ii,:)-goal_sub));
% H(neighbour_ind)=g(neighbour_ind)+norm((neighbour(ii,:)-goal_sub));
        end
    end
end
